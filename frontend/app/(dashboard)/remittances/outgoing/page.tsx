...
import { REMITTANCE_STATUS_COLORS, REMITTANCE_STATUS_LABELS } from '@/src/models/remittance';
import Link from 'next/link';

export default function OutgoingRemittancesList() {
    const router = useRouter();
    const [remittances, setRemittances] = useState<OutgoingRemittance[]>([]);
    const [filteredRemittances, setFilteredRemittances] = useState<OutgoingRemittance[]>([]);
    ...
    const loadRemittances = async () => {
        try {
            setLoading(true);
            const data = await remittanceApi.getOutgoing();
            setRemittances(data);
        } catch (err: any) {
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto">
            ...
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900">📤 Outgoing Remittances</h1>
                    <p className="text-gray-600 mt-1">Canada → Iran Money Transfers</p>
                </div>
                <Link
            ...
            {filteredRemittances.length === 0 && (
                <div className="bg-white rounded-lg shadow p-12 text-center">
                    ...
                    <h3 className="text-xl font-semibold text-gray-900 mb-2">
                        {remittances.length === 0 ? 'No Outgoing Remittances' : 'No Results Found'}
                    </h3>
                    <p className="text-gray-600 mb-6">
                        {remittances.length === 0
                            ? 'Create your first outgoing remittance to get started'
                                    ...
                                    return (
                                        <tr key={remittance.id} className="hover:bg-gray-50 cursor-pointer">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="font-medium text-gray-900">{remittance.recipientName}</div>
                                                <div className="text-sm text-gray-500">Buy: {remittance.buyRateCad.toLocaleString()}</div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="w-full bg-gray-200 rounded-full h-2 mb-1">
                                                    <div
                                                        className={`h-2 rounded-full ${progress === 100 ? 'bg-green-600' : progress > 0 ? 'bg-yellow-500' : 'bg-red-500'
                                                            }`}
                                                        style={{ width: `${progress}%` }}
                                                    ></div>
                                                </div>
                                                <div className="text-xs text-gray-600">{progress.toFixed(0)}% settled</div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                ...
                <div className="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="bg-white rounded-lg shadow p-4">
                        ...
                        <div className="text-xl font-bold text-green-600">
                            {remittanceApi.formatCurrency(
                                filteredRemittances.reduce((sum, r) => sum + r.settledAmountIrr, 0),
                                'IRR'
                            )}
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-lg shadow p-4">
                        ...
                        <div className={`text-xl font-bold ${filteredRemittances.reduce((sum, r) => sum + r.totalProfitCad, 0) >= 0 ? 'text-green-600' : 'text-red-600'
                            }`}>
                            {remittanceApi.formatCurrency(
                                filteredRemittances.reduce((sum, r) => sum + r.totalProfitCad, 0)
                            )}
                        </div>
                    </div>
                </div>
